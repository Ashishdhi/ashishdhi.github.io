...
import defaultCoinUSDT from '../images/coin_dollar.svg';
...
export default function CoinApp(props) {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      setMiningInfo(prevMiningInfo => {
        ...
        if (userData.id)  axios.get(`${domainname}/api/user/${userData.id}`)
  ...
  useEffect(() => {
    const req = async () => {
      await axios.post(`${domainname}/api/user/${userId}/add-point`, {
  ...
  useEffect(() => {
    const req = async () => {
      await axios.get(`${domainname}/api/user/${userId}`)
      .then(response => {
      ...
      await axios.get(`${domainname}/api/user/${userId}/get-rank`)
      .then(response => {
      ...
      await axios.get(`${domainname}/api/leaderboard`)
      .then(response => {
  ...
  const handleWithdraw = () => {
    if (pointCount >= 50) {
      axios.post(`${domainname}/api/withdraw`, {
        userId: userId,
  ...
  position: 'absolute',
  top: '20%',
  left: getPointLeftPosition(pointCount)-30,
  color: 'aliceblue',
  ...
  alignItems: 'center', // Add this to vertically center the image and text
}}>
  <img src={defaultCoinUSDT} alt="Coin" style={{ width: '30px', height: '30px', marginRight: '15px' }} />
  {pointCount}
</Typography>
      ...
      {/* Buttons */}
      <Box sx={{ mb: 2, width:'95vw', maxWidth:'138vw', background: '#232323', borderRadius:'15px', display: 'flex', backdropFilter: 'blur(10px)', justifyContent: 'space-around', alignItems: 'center'}}>
